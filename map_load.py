...
class Map:
    def __init__(self, loading_map="default", count=0):
        load_map, name = code.load_map(loading_map)
        if load_map["T"] == "True":
            self.name = T[name]
        else:
            self.name = name
        self.map_dict = init.Config["MAP"]["map dict"]
    ...
    def loading_map(self, loading_map="default", count=0):
        load_map, name = code.load_map(loading_map)
        if load_map["T"] == "True":
            self.name = T[name]
        else:
