...
enum GAME_TURN_TYPE { MOVE, FIGHT }

# --- 导出变量 ---
@export_group("References")
@export var characters_paths: Array[NodePath]
@export var move_hint_layer_path: NodePath
@export var tile_map_layer_path: NodePath
@export var ex_map_layer_path: NodePath
@export var ui_name_container_path: NodePath
@export var ui_info_container_path: NodePath
...
@export var fight_type_roll: Array[Array] = [
	...
	[FIGHT_OPERATION.X_RAY, 1],
	[FIGHT_OPERATION.POWER_SHOT, 1],
	[FIGHT_OPERATION.BOMB, 0],
	[FIGHT_OPERATION.HEALTH, 1]
]
...
var turn_confirm_skip: bool = false

# 当前回合临时数据
var current_move_type: int
var keep_move_type: bool = false
var current_fight_type: int
var keep_fight_type: bool = false
var switch_character_flag: bool = true
...
# 战斗缓存
var valid_attack_targets: Array[Node] = []
...
func _init_nodes():
	if move_hint_layer_path: _move_layer = get_node(move_hint_layer_path)
	if tile_map_layer_path: _map_layer = get_node(tile_map_layer_path)
	if ex_map_layer_path: _ex_layer = get_node(ex_map_layer_path)
	if ui_name_container_path: _ui_name_container = get_node(ui_name_container_path)
	if ui_info_container_path: _ui_info_container = get_node(ui_info_container_path)
...
func _map_init():
	...
	# 初始化特殊地块列表
	special_tiles["cover"] = _map_layer.get_used_cells_by_id(0, MAP_LAYER_ID.COVER, 0)
	special_tiles["heal"] = _map_layer.get_used_cells_by_id(0, MAP_LAYER_ID.HEAL, 0)
	special_tiles["cannon"] = _map_layer.get_used_cells_by_id(0, MAP_LAYER_ID.CANNON, 0)
	special_tiles["speed"] = _map_layer.get_used_cells_by_id(0, MAP_LAYER_ID.SPEED, 0)
	special_tiles["capture"] = _map_layer.get_used_cells_by_id(1, MAP_LAYER_ID.CAPTURE, 0)
...
func _start_move_turn():
	...
	if not keep_move_type:
		...
		current_move_type = MOVE_OPERATION[key_list[weighted_random(move_type_roll)]]
	else:
	...
	match current_move_type:
		MOVE_OPERATION.CROSS, MOVE_OPERATION.DOUBLE_CROSS, MOVE_OPERATION.DIAGONAL:
			if current_move_type == MOVE_OPERATION.DOUBLE_CROSS:
				m_move_range *= 2
				
			var dirs: Array 
			if current_move_type == MOVE_OPERATION.DIAGONAL:
			...
			for dir in dirs:
				for step in range(1, m_move_range + 1):
					...
					var cell_color = ATLAS_COORDS.BLUE_MOVE
					if p in forbidden_list:
						cell_color = ATLAS_COORDS.GRAY_FORBIDDEN
					if p in other_chars_pos:
						cell_color = ATLAS_COORDS.RED_ATTACK
	...
	if current_active_char.tp_position_set:
		...
		if origin != current_active_char.tp_position and current_active_char.tp_position not in other_chars_pos:
			...
			if tp_pos != null and tp_pos in used_cells:
				_move_layer.set_cell(tp_pos, 2, ATLAS_COORDS.BLUE_MOVE)
			_move_layer.set_cell(origin, 2, ATLAS_COORDS.BLUE_MOVE)
		...
		for x in range(-tp_range, tp_range + 1):
			for y in range(-tp_range, tp_range + 1):
				...
				if p in used_cells and current_active_char._can_move_to(p):
					_move_layer.set_cell(p, 2, ATLAS_COORDS.PURPLE_TELEPORT)
...
func _handle_click_input():
	...
	if turn_type == GAME_TURN_TYPE.MOVE:
		...
		# 传送设置点
		if cell_atlas == ATLAS_COORDS.PURPLE_TELEPORT:
			_confirm_action()
		...
		# 普通移动
		if cell_atlas == ATLAS_COORDS.BLUE_MOVE or cell_atlas == ATLAS_COORDS.GRAY_FORBIDDEN:
			_confirm_action()
		...
		if cell_atlas == ATLAS_COORDS.RED_ATTACK:
