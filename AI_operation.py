...
def AI_move(ai, player, again=None):
    GAMING.display_statue("Please throw!", code.SCREEN)
    ...
    if ai.HP <= 3 and ai.DEF_prop + ai.DEF_dice + ai.DEF_ai < 1:
        ...
        if distance[0] in (1, 2) or distance[1] in (1, 2):
            ...
            if player.HP <= 3 and code.Config["MAP"]["map 1"]["list pos"]["cannon dot"][0] in pos_list:
                return_pos_list.append(code.Config["MAP"]["map 1"]["list pos"]["cannon dot"][0])
                return_pos_list.append(code.Config["MAP"]["map 1"]["list pos"]["cannon dot"][0])
            # 假如周围可以回血，增加选择权重
            if code.Config["MAP"]["map 1"]["list pos"]["recovery dot"][0] in pos_list:
                return_pos_list.append(code.Config["MAP"]["map 1"]["list pos"]["recovery dot"][0])
            if code.Config["MAP"]["map 1"]["list pos"]["recovery dot"][1] in pos_list:
                return_pos_list.append(code.Config["MAP"]["map 1"]["list pos"]["recovery dot"][1])
    ...
    # 如果自身血量充足的情况
        ...
        # 如果对方有充足血量
            ...
            if player.pos[0] == code.Config["MAP"]["map 1"]["list pos"]["cannon dot"][0]:
                for i in pos_list:
                    distance = operation.calculate_distance(i, player.pos[0])
                    if distance[0] not in (1, 2) and distance[1] not in (1, 2):
                        return_pos_list.append(distance)
